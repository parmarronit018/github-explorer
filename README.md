...
- Save/Remove toggle — theme-aware (white filled in dark mode, black filled in light mode)
...
- Empty state with icon, message, and "Search Users" button

### UI / UX
- Dark / Light mode toggle with sun/moon SVG icons
- All colors adapt to theme — GitHub design system color palette
- Active tab (Search / Watchlist) shown as filled button; inactive as plain text
- Consistent button styles throughout — View, Remove, Save, Search all theme-aware
- Responsive layout — works on mobile, tablet, and desktop
- Navbar stays on one line on all screen sizes (no wrapping)
- Footer always stays at the bottom (flex layout)
...
- React 19 (Vite)
- Tailwind CSS v4
- GitHub REST API (public, no token required)

## What I'd Improve With More Time

- Add a GitHub personal access token input to raise the API rate limit from 60 to 5,000 req/hr
- Show a user's pinned repositories (requires GitHub GraphQL API)
- Show user organizations on the profile card (`/users/{username}/orgs`)
- Add a "recent searches" list that persists in localStorage across refreshes
- Add sorting/filtering options to the repo list (by language, fork count, etc.)
- Add subtle animations on card entry (e.g. fade-in)
...
- GitHub's API rate limit (60 req/hr unauthenticated) returns a `403`, not a `429` — had to handle both status codes to show the right error message
